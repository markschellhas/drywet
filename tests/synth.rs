...
fn synth_trigger_attack_release_mixes_nonzero_pcm() {
    ...
    synth
        .trigger_attack_release(&mut ctx, "A4", 0.05, Some(0.0.into()))
        .unwrap();
...
fn synth_polyphony_cap_and_release_all() {
    ...
    synth
        ...
        .unwrap();
    synth
        .trigger_attack(&mut ctx, "E4", Some(0.0.into()))
        .unwrap();
    assert!(synth
        .trigger_attack(&mut ctx, "G4", Some(0.0.into()))
        .is_err());
...
fn synth_trigger_attack_release_none_mixes_at_write_cursor() {
    ...
    synth.release_all(None);
    synth.trigger_attack(&mut ctx, "C4", None).unwrap();
}
